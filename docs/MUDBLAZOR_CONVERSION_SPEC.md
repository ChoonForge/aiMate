...
### 🧩 Global vs. User-Created Content

| Feature                   | Global                  | Scoped Override             | Notes                                            |
| ------------------------- | ----------------------- | --------------------------- | ------------------------------------------------ |
| Tags (e.g. feedback tags) | ✅ Yes                   | ✅ User-defined + re-usable  | Global ones appear in dropdown, user can add own |
| Custom Prompts            | ✅ Admin-defined presets | ✅ Saved per-user or per-org | Promote popular ones to global                   |
| Structured Forms          | ✅ Admin-defined         | ✅ User-creatable (dev tier) | Templating planned                               |
| Chat Shortcuts / Macros   | ✅ Yes                   | ✅ User + Org level          | Think: slash commands or pinned macros           |
| Tools / Plugins           | ✅ Public or Private     | ✅ Org-scoped                | Developer tier gets publishing rights            |

All user-created config is:

* **Stored relationally** (no "dumped JSON blackboxes")

* **Searchable + referenceable** across users/orgs

* **Promotable** to global by admin



### 🔄 1. **Connection Health Checks**

> Validate connection URLs before saving

* **Add:** Test button in connection dialog (ping `/models` or `/health`)

* **Component:** `MudButton` + loading spinner + inline status icon (✅/❌)

* **Bonus:** Auto-detect provider type from response

* * *

### 🧩 2. **Plugin Toggle Support**

> Not just enable/disable — but full per-model or per-role plugin scoping

* Add role-aware plugin toggles (e.g., only Admins can use Web Search)

* Plugin discovery via dynamic manifest fetch (if supporting remote plugins)

* * *

### 🔗 3. **Conversation Linking**

> For attachments, prompt reuse, previous context injection
...
* UI: Modal or bottom sheet with quick search + `MudList` of recent items

* * *

### 🎯 4. **Model Targeting Rules**

> Scope models to certain users, groups, or use cases

* Use cases:
  
  * Claude for summarization
  
  * GPT-4 Turbo for development

* UI: `MudSelectChipGroup` for tags like `Group: Admins`, `Task: Summary`

* * *

### 🧠 5. **Prompt Templates / Snippets**

> Reusable prompt fragments across models

* Taggable, saved snippets (stored per user or global)

* Hotkey injection (like `/summarize`)

* `MudDialog` or `MudMenu` to select from library

* * *

### 📝 6. **Audit Log / Admin Feed**

> Trace changes to models, users, prompts, etc.

* Display with:
  
  * `MudTimeline` or vertical `MudList`
  
  * Filter by action type, date, user

* Storage: Lightweight log entries via `IAuditLogService`
...
| --------------------------------- | ------------------------------------------------------ |
| **Inline toggle animations**      | `MudSwitch` with `OnChanged` event & fade feedback     |
| **Chip groups for tag selection** | `MudChipSet` with filterable chips                     |
| **Searchable dropdowns**          | `MudSelect TItem` with `SearchFunc` and `ToStringFunc` |
| **Mini alert toasts**             | `ISnackbar.Add(...)` with position, duration           |
| **Keyboard shortcuts**            | `@onkeydown` with Blazor event binding                 |
...
```csharp
public class ModelAdvancedSettings
{
...
## ✏️ 4. Prompt Overrides

```csharp
public class PromptOverrides
{
    public string? TitleGeneration { get; set; }
    public string? TagsGeneration { get; set; }
    public string? FollowUpGeneration { get; set; }
    public string? WebSearchQuery { get; set; }
    public string? RetrievalQuery { get; set; }
    public string? ImagePromptGeneration { get; set; }
    public string? QueryGeneration { get; set; }
    public string? ToolsFunctionCall { get; set; }
}
```

---

## 🎛️ 5. User Interface Settings

```csharp
public class UISettings
{
    public bool HighContrastMode { get; set; }
    public bool ShowChatTitleInTab { get; set; }
    public bool NotificationSound { get; set; }
    public bool AllowLocation { get; set; }
    public bool HapticFeedback { get; set; }
    public bool CopyFormattedText { get; set; }
    public bool ToastForUpdates { get; set; }
    public bool ShowWhatsNewOnLogin { get; set; }

    // Chat-specific
    public string ChatDirection { get; set; } = "Auto";
    public string LandingMode { get; set; } = "Default";
    public bool ChatBubbleUI { get; set; }
    public bool ShowUsernameInsteadOfYou { get; set; }
    public string? ChatBackgroundImageUrl { get; set; }
}
```
...
<MudButton Color="Color.Error" Variant="Variant.Filled" OnClick="ClearAllConversations">
...
public class UserProfile
{
    ...
    public string? AssistantName { get; set; } = "Echo";
...
public class PromptSnippet
{
    ...
    public string Scope { get; set; } = "User"; // Global, Group, User, Role
...
public class TagDefinition
{
    ...
    public string? OwnerId { get; set; }
    public string? UsageContext { get; set; } // e.g. Feedback
}
```
...
public class FeatureGate
{
    ...
    public bool EnabledGlobally { get; set; } = true;
}
```

---

## ⚙️ 12. Scoped Settings Service
...
## 📦 13. UI Reuse Patterns

| Component                   | Purpose                            |
| --------------------------- | ---------------------------------- |
| `BaseDialog.razor`          | Consistent modals                  |
| `TabbedPanel.razor`         | Sectioned UX (e.g. Model Settings) |
| `PromptLibraryDialog.razor` | Snippet discovery                  |
| `FeatureGateToggle.razor`   | Admin toggle per feature           |
| `TagSelector.razor`         | Auto-complete + scoped filter      |
| `AttachMenu.razor`          | Floating expandables               |

---

## ✨ 14. Open Source Polish (Optional)

| Feature               | Description                             |
| --------------------- | --------------------------------------- |
| 🧪 Prompt Preview     | Test prompt via model                   |
| 📊 Tag Usage Heatmap  | Visual admin analytics                  |
| 🔁 Prompt Chaining    | Link responses                          |
| 🧼 Auto-purge         | Expired prompts/tags                    |
| 🧠 Suggested Snippets | Based on usage or admin pin             |
| ⏳ Versioned Prompts   | Editable history w/ rollback (OSS tier) |
